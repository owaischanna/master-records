...
export function mapReportRow(row) {
  ...
  const deposit_slip_number = row.deposit_slip_number ?? row['Deposit Slip Number'] ?? row['Deposit Slip'] ?? null;
  const deposit_date = row.deposit_date ?? row['Deposit Date'] ?? row['Deposit date'] ?? null;
  const deposit_status = row.deposit_status ?? row['Deposit Status'] ?? null;
  const beneficiary_status = row.beneficiary_status ?? row['Beneficiary Status'] ?? row.beneficary_status ?? row.Status ?? row.status ?? null;
  ...
  return {
    ...row,
    s_no, full_name, father_husband_name, cnic, village_name, union_council,
    business_plan, amount, recovery_amount, deposit_amount, receipt_no, date,
    deposit_slip_number, deposit_date, deposit_status, beneficiary_status,
    'S.#': s_no,
    'Beneficary Name': full_name,
    'Beneficiary Name': full_name,
    "Father's/Husband's Name": father_husband_name,
...
export function normalizeReportPayload(updates, month) {
  // Converts empty strings to null for numeric columns (prevents bigint error)
  ...
  // Converts empty strings to null for text columns
  ...
  const normalized = {
    ...(month ? { month } : {}),
    s_no:                toStrOrNull(updates.s_no ?? updates['S.#'] ?? updates.sno ?? updates.s ?? updates.id),
    full_name:           toStrOrNull(updates.full_name ?? updates['Beneficary Name'] ?? updates['Beneficiary Name'] ?? updates['Full Name'] ?? updates.name),
    father_husband_name: toStrOrNull(updates.father_husband_name ?? updates["Father's/Husband's Name"] ?? updates['Father Husband Name']),
    cnic:                toStrOrNull(updates.cnic ?? updates['CNIC Number'] ?? updates['CNIC']),
    village_name:        toStrOrNull(updates.village_name ?? updates['Village Name'] ?? updates['Village Name/Parro']),
    union_council:       toStrOrNull(updates.union_council ?? updates['Union Council Name'] ?? updates['Union Council']),
    business_plan:       toStrOrNull(updates.business_plan ?? updates['Investment (Business Plan)'] ?? updates['Investment']),
    amount:              toNumOrNull(updates.amount ?? updates['Amount']),
    recovery_amount:     toNumOrNull(updates.recovery_amount ?? updates['Recovery Amount'] ?? updates['Total Received']),
    receipt_no:          toStrOrNull(updates.receipt_no ?? updates['Receipt #'] ?? updates.receipt_number),
    date:                toStrOrNull(updates.date ?? updates['Date']),
    deposit_slip_number: toStrOrNull(updates.deposit_slip_number ?? updates['Deposit Slip Number']),
    deposit_date:        toStrOrNull(updates.deposit_date ?? updates['Deposit Date']),
    deposit_amount:      toNumOrNull(updates.deposit_amount ?? updates['Deposit Amount']),
    deposit_status:      toStrOrNull(updates.deposit_status ?? updates['Deposit Status']),
    beneficiary_status:  toStrOrNull(updates.beneficiary_status ?? updates['Beneficiary Status'] ?? updates['Status']),
  };

  // Remove null/undefined keys so Supabase doesn't try to write them
  Object.keys(normalized).forEach((key) => {
    if (normalized[key] === null || normalized[key] === undefined) {
      delete normalized[key];
    }
